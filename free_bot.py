...
class Database:
    ...
    def get_user(self, user_id):
        conn = self.get_conn()
        c = conn.cursor()
        c.execute('''SELECT user_id, phone, session_string,
    ...
    def logout_user(self, user_id):
        conn = self.get_conn()
        c = conn.cursor()
        c.execute('''UPDATE free_users SET phone=NULL, session_string=NULL,
...
class UzeronFreeBot:
        ...
        while True:
            ...
            try:
                ...
                for uid, username in users:
                    ...
                    if not user or not user[2]:  # no session_string
                        continue
                    if not user[9]:  # branding_set
                        continue
    ...
    async def verify_branding(self, uid, user):
        try:
            # user[2] = session_string
            user_client = TelegramClient(
                StringSession(user[2]), BOT_API_ID, BOT_API_HASH)
    ...
    def register_handlers(self):
        ...
        async def callbacks(event):
            ...
            if data == 'dashboard':
                ...
                # user indices: 0=user_id, 1=phone, 2=session_string, 3=promo_message,
                #               4=is_active, 5=runtime_today, 6=last_reset,
                #               7=warning_count, 8=is_banned, 9=branding_set
                phone = user[1] if user and user[1] else "Not connected"
                connected = "✅ Connected" if user and user[2] else "❌ Not connected"
                branding = "✅ Set" if user and user[9] else "⏳ Pending"
                edit_msg(uid, mid,
                         f"👤 <b>My Account</b>\n\n"
                ...
                s = "🟢 Live" if user and user[4] else "🔴 Stopped"
                msg_preview = (user[3][:60]+'...') if user and user[3] and len(user[3]) > 60 else (user[3] if user else "Not set") or "Not set"
                ...
                if not user or not user[2]:  # no session_string
                    ...
                    return
                if not user[3]:  # no promo_message
                ...
                if user and user[2]:  # already has session_string
                    await event.answer("✅ Already logged in!", alert=True)
                    return
                # Start login — phone number ONLY, no API ID/Hash needed
    ...
    # ──────────────────────────────────────────────
    # LOGIN FLOW: Phone → OTP (→ 2FA if needed)
    # No API ID / API Hash from user — uses backend creds
    # ──────────────────────────────────────────────
    async def handle_login(self, event, uid, text):
        ...
        if step == 'waiting_phone':
            ...
            try:
                ...
                # Show numpad keyboard for OTP entry
                send_msg(uid,
                         "📨 <b>OTP Sent!</b>\n\n"
                         "Enter the verification code Telegram sent you:\n\n"
                         "🔢 <i>Numbers only, e.g. <code>12345</code></i>",
                         make_keyboard([
                             [{"text": "1", "callback_data": f"otp_1_{uid}"},
                              {"text": "2", "callback_data": f"otp_2_{uid}"},
                              {"text": "3", "callback_data": f"otp_3_{uid}"}],
                             [{"text": "4", "callback_data": f"otp_4_{uid}"},
                              {"text": "5", "callback_data": f"otp_5_{uid}"},
                              {"text": "6", "callback_data": f"otp_6_{uid}"}],
                             [{"text": "7", "callback_data": f"otp_7_{uid}"},
                              {"text": "8", "callback_data": f"otp_8_{uid}"},
                              {"text": "9", "callback_data": f"otp_9_{uid}"}],
                             [{"text": "⌫ Del", "callback_data": f"otp_del_{uid}"},
                              {"text": "0", "callback_data": f"otp_0_{uid}"},
                              {"text": "✅ Submit", "callback_data": f"otp_submit_{uid}"}],
                             [{"text": "❌ Cancel", "callback_data": "cancel_login"}]
                         ]))
                # Store OTP buffer
                self.login_states[uid]['otp_buffer'] = ''
                self.login_states[uid]['otp_msg_id'] = None
            except Exception as e:
                send_msg(uid, f"❌ <b>Error sending code:</b>\n<code>{e}</code>\n\nCheck your number and try again.")
                if uid in self.login_states:
                    del self.login_states[uid]

        elif step == 'waiting_code':
            # User typed code manually (fallback to text entry)
            code = text.replace('-', '').replace(' ', '')
            if not code.isdigit():
                send_msg(uid, "❌ Enter only the numeric OTP code")
                return
            await self._submit_otp(uid, code)
            ...
            try:
                ...
                send_msg(uid, f"❌ <b>2FA failed:</b> {e}\n\nTry again.")
                await state['client'].disconnect()
                del self.login_states[uid]

    async def _submit_otp(self, uid, code):
        ...
        try:
            ...
            send_msg(uid,
                     ...
                     "Please type your 2FA password and send it:",
                     make_keyboard([[{"text": "❌ Cancel",
                                      "callback_data": "cancel_login"}]]))
        except Exception as e:
            send_msg(uid, f"❌ <b>Invalid code:</b> {e}\n\nTry again.")
            try:
                await state['client'].disconnect()
            except: pass
            del self.login_states[uid]
    ...
    # ──────────────────────────────────────────────
    # OTP NUMPAD CALLBACK HANDLER
    # Handles button presses: otp_0 .. otp_9, otp_del, otp_submit
    # ──────────────────────────────────────────────
    async def handle_otp_button(self, event, uid, action):
        state = self.login_states.get(uid)
        if not state or state.get('step') != 'waiting_code':
            await event.answer("Session expired. Please login again.", alert=True)
            return

        buf = state.get('otp_buffer', '')

        if action == 'del':
            buf = buf[:-1]
        elif action == 'submit':
            if not buf:
                await event.answer("❌ Enter the code first!", alert=True)
                return
            await event.answer("⏳ Verifying...")
            await self._submit_otp(uid, buf)
            return
        else:
            if len(buf) < 6:
                buf += action

        state['otp_buffer'] = buf

        display = buf if buf else "_ _ _ _ _"
        await event.answer()
        # Edit the numpad message to show current input
        edit_msg(uid, event.query.msg_id,
                 f"📨 <b>OTP Sent!</b>\n\n"
                 f"🔢 Code entered: <code>{display}</code>\n\n"
                 f"Press <b>✅ Submit</b> when done:",
                 make_keyboard([
                     [{"text": "1", "callback_data": f"otp_1_{uid}"},
                      {"text": "2", "callback_data": f"otp_2_{uid}"},
                      {"text": "3", "callback_data": f"otp_3_{uid}"}],
    ...
    async def run_campaign(self, uid):
        try:
            ...
            if not groups:
                send_msg(uid, "❌ <b>No groups found!</b> Join some groups first.")
                self.db.set_campaign_status(uid, 0)
                await user_client.disconnect()
                return
            ...
            while self.db.get_user(uid)[4]:  # is_active = index 4
                runtime = self.db.get_runtime_today(uid)
                ...
                for group in groups:
                    if not self.db.get_user(uid)[4]: break  # is_active
